...
exports.projecttasksetting = async (req, res) => {
    try {
        ...
        // Calculate the number of pending tasks for the current space
        const pendingTaskCount = await Task.countDocuments({
            project: spaceId,
            taskStatus: 'pending' // Adjust 'pending' to match the status used in your schema
        });
        ...
        res.render('task/task-setting', {
            ...
            pendingTaskCount // Pass the pending task count to the template
        });
    } catch (error) {
        console.error(error);
